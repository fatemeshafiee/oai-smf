...
void release_ue_session(std::set <std::pair<int, int>>  toBanSessIDs){
  for (auto ids : toBanSessIDs) {
    ...
    if (!pc.get()->find_pdu_session(ids.first + 1, sp)) {
      // error
    ...
    if (proc->get_next_upf(dl_edges, ul_edges, current_upf) ==
      ...
      Logger::nwdaf_sub().warn("[FATEMEH] Can not find UPF!");
      continue ;
    }
...
void manage_suspicious_session(std::vector<UEPduRatioPair> ueRatioList){
  ...
  for (int i=0; i<ueRatioList.size(); i++){
    if(ueRatioList[i].ratio>Treshold){
      toBanSessIDs.insert(std::make_pair(ueRatioList[i].pduSessId, ueRatioList[i].seId));
      Logger::nwdaf_sub().warn("[FATEMEH] The Ids are: %d %d", ueRatioList[i].pduSessId, ueRatioList[i].seId);
