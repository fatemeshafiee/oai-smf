...
void smf_n1_n2::create_n1_sm_container(pdu_session_msg& msg, uint8_t n1_msg_type, std::string& nas_msg_str, cause_value_5gsm_e sm_cause)
{
  ...
  case PDU_SESSION_ESTABLISHMENT_ACCEPT: {
    ...
    Logger::smf_app().debug("PDUAddress: %x %x %x %x %x",
        ...
        (unsigned char)(sm_msg->pdu_session_establishment_accept.pduaddress.pdu_address_information->data[0]),
        (unsigned char)(sm_msg->pdu_session_establishment_accept.pduaddress.pdu_address_information->data[1]),
    ...
    Logger::smf_app().debug("n1MessageContent (%d bytes), %s ", bytes, nas_msg_str.c_str());
...
void smf_n1_n2::create_n2_sm_information(pdu_session_msg& msg, uint8_t ngap_msg_type, n2_sm_info_type_e ngap_ie_type, std::string& ngap_msg_str)
{
  ...
  case n2_sm_info_type_e::PDU_RES_SETUP_RSP: {
    ...
    if(er.encoded < 0)
    {
      Logger::smf_app().warn("[Create N2 SM Information] NGAP PDU Session Resource Setup Resonse Transfer encode failed, er.encoded: %d", er.encoded);
      return;
    }
