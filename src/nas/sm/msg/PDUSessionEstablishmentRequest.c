...
int decode_pdu_session_establishment_request( pdu_session_establishment_request_msg *pdu_session_establishment_request, uint8_t* buffer, uint32_t len)
{
  ...
  while (len - decoded > 0) {
    ...
    case PDU_SESSION_ESTABLISHMENT_REQUEST_EPCO_IEI:
      if ((decoded_result = decode_extended_protocol_configuration_options (&pdu_session_establishment_request->extendedprotocolconfigurationoptions, PDU_SESSION_ESTABLISHMENT_REQUEST_EPCO_IEI, buffer + decoded, len - decoded)) < 0)
        return decoded_result;
      else {
        decoded += decoded_result;
...
int encode_pdu_session_establishment_request( pdu_session_establishment_request_msg *pdu_session_establishment_request, uint8_t* buffer, uint32_t len)
{
  ...
  if ((pdu_session_establishment_request->presence & PDU_SESSION_ESTABLISHMENT_REQUEST_EPCO_PRESENT) == PDU_SESSION_ESTABLISHMENT_REQUEST_EPCO_PRESENT) {
    if ((encoded_result = encode_extended_protocol_configuration_options (pdu_session_establishment_request->extendedprotocolconfigurationoptions, PDU_SESSION_ESTABLISHMENT_REQUEST_EPCO_IEI, buffer + encoded,len - encoded)) < 0)
      return encoded_result;
    else
      encoded += encoded_result;
  }
