  ...
  while (len - decoded > 0) {
    ...
    switch (ieiDecoded & 0xf0) {
      case PDU_SESSION_ESTABLISHMENT_REQUEST_PDU_SESSION_TYPE_IEI:
        if ((decoded_result = decode__pdu_session_type(&pdu_session_establishment_request->_pdusessiontype, PDU_SESSION_ESTABLISHMENT_REQUEST_PDU_SESSION_TYPE_IEI, buffer + decoded, len - decoded))
            < 0)
          return decoded_result;
        else {
          decoded += decoded_result;
      ...
      case PDU_SESSION_ESTABLISHMENT_REQUEST_ALWAYSON_PDU_SESSION_REQUESTED_IEI:
        if ((decoded_result = decode_alwayson_pdu_session_requested(&pdu_session_establishment_request->alwaysonpdusessionrequested,
                                                                    PDU_SESSION_ESTABLISHMENT_REQUEST_ALWAYSON_PDU_SESSION_REQUESTED_IEI, buffer + decoded, len - decoded)) < 0)
          return decoded_result;
        else {
          decoded += decoded_result;
    ...
    switch (ieiDecoded) {
      case PDU_SESSION_ESTABLISHMENT_REQUEST__5GSM_CAPABILITY_IEI:
        if ((decoded_result = decode__5gsm_capability(&pdu_session_establishment_request->_5gsmcapability, PDU_SESSION_ESTABLISHMENT_REQUEST__5GSM_CAPABILITY_IEI, buffer + decoded, len - decoded))
            < 0)
          return decoded_result;
        else {
          decoded += decoded_result;
      ...
      case PDU_SESSION_ESTABLISHMENT_REQUEST_MAXIMUM_NUMBER_OF_SUPPORTED_IEI:
        if ((decoded_result = decode_maximum_number_of_supported_packet_filters(&pdu_session_establishment_request->maximumnumberofsupportedpacketfilters,
                                                                                PDU_SESSION_ESTABLISHMENT_REQUEST_MAXIMUM_NUMBER_OF_SUPPORTED_IEI, buffer + decoded, len - decoded)) < 0)
          return decoded_result;
        else {
          decoded += decoded_result;
      ...
      case PDU_SESSION_ESTABLISHMENT_REQUEST_SMPDUDN_REQUEST_CONTAINER_INFORMATION_IEI:
        if ((decoded_result = decode_smpdudn_request_container(&pdu_session_establishment_request->smpdudnrequestcontainer, PDU_SESSION_ESTABLISHMENT_REQUEST_SMPDUDN_REQUEST_CONTAINER_INFORMATION_IEI,
      ...
      case PDU_SESSION_ESTABLISHMENT_REQUEST_EPCO_IEI:
        if ((decoded_result = decode_extended_protocol_configuration_options(&pdu_session_establishment_request->extendedprotocolconfigurationoptions, PDU_SESSION_ESTABLISHMENT_REQUEST_EPCO_IEI,
                                                                             buffer + decoded, len - decoded)) < 0)
          return decoded_result;
        else {
          decoded += decoded_result;
          pdu_session_establishment_request->presence |= PDU_SESSION_ESTABLISHMENT_REQUEST_EPCO_PRESENT;
        }
        break;
    }
  }
...
int encode_pdu_session_establishment_request(pdu_session_establishment_request_msg *pdu_session_establishment_request, uint8_t *buffer, uint32_t len) {
  ...
  if ((pdu_session_establishment_request->presence & PDU_SESSION_ESTABLISHMENT_REQUEST_MAXIMUM_NUMBER_OF_SUPPORTED_PRESENT) == PDU_SESSION_ESTABLISHMENT_REQUEST_MAXIMUM_NUMBER_OF_SUPPORTED_PRESENT) {
    if ((encoded_result = encode_maximum_number_of_supported_packet_filters(pdu_session_establishment_request->maximumnumberofsupportedpacketfilters,
                                                                            PDU_SESSION_ESTABLISHMENT_REQUEST_MAXIMUM_NUMBER_OF_SUPPORTED_IEI, buffer + encoded, len - encoded)) < 0)
      return encoded_result;
    else
      encoded += encoded_result;
  }

  if ((pdu_session_establishment_request->presence & PDU_SESSION_ESTABLISHMENT_REQUEST_ALWAYSON_PDU_SESSION_REQUESTED_PRESENT)
      == PDU_SESSION_ESTABLISHMENT_REQUEST_ALWAYSON_PDU_SESSION_REQUESTED_PRESENT) {
    if ((encoded_result = encode_alwayson_pdu_session_requested(pdu_session_establishment_request->alwaysonpdusessionrequested, PDU_SESSION_ESTABLISHMENT_REQUEST_ALWAYSON_PDU_SESSION_REQUESTED_IEI,
  ...
  if ((pdu_session_establishment_request->presence & PDU_SESSION_ESTABLISHMENT_REQUEST_SMPDUDN_REQUEST_CONTAINER_INFORMATION_PRESENT)
      == PDU_SESSION_ESTABLISHMENT_REQUEST_SMPDUDN_REQUEST_CONTAINER_INFORMATION_PRESENT) {
    if ((encoded_result = encode_smpdudn_request_container(pdu_session_establishment_request->smpdudnrequestcontainer, PDU_SESSION_ESTABLISHMENT_REQUEST_SMPDUDN_REQUEST_CONTAINER_INFORMATION_IEI,
                                                           buffer + encoded, len - encoded)) < 0)
      return encoded_result;
    else
      encoded += encoded_result;
  ...
  if ((pdu_session_establishment_request->presence & PDU_SESSION_ESTABLISHMENT_REQUEST_EPCO_PRESENT) == PDU_SESSION_ESTABLISHMENT_REQUEST_EPCO_PRESENT) {
    if ((encoded_result = encode_extended_protocol_configuration_options(pdu_session_establishment_request->extendedprotocolconfigurationoptions, PDU_SESSION_ESTABLISHMENT_REQUEST_EPCO_IEI,
                                                                         buffer + encoded, len - encoded)) < 0)
      return encoded_result;
    else
      encoded += encoded_result;
