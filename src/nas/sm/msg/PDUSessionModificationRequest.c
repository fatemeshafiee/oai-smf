  ...
  while (len - decoded > 0) {
    ...
    switch (ieiDecoded & 0xf0) {
      case PDU_SESSION_MODIFICATION_REQUEST_ALWAYSON_PDU_SESSION_REQUESTED_IEI:
        if ((decoded_result = decode_alwayson_pdu_session_requested(&pdu_session_modification_request->alwaysonpdusessionrequested, PDU_SESSION_MODIFICATION_REQUEST_ALWAYSON_PDU_SESSION_REQUESTED_IEI,
    ...
    switch (ieiDecoded) {
      ...
      case PDU_SESSION_MODIFICATION_REQUEST_MAXIMUM_NUMBER_OF_SUPPORTED_IEI:
        if ((decoded_result = decode_maximum_number_of_supported_packet_filters(&pdu_session_modification_request->maximumnumberofsupportedpacketfilters,
                                                                                PDU_SESSION_MODIFICATION_REQUEST_MAXIMUM_NUMBER_OF_SUPPORTED_IEI, buffer + decoded, len - decoded)) < 0)
          return decoded_result;
        else {
          decoded += decoded_result;
      ...
      case PDU_SESSION_MODIFICATION_REQUEST_INTGRITY_PROTECTION_MAXIMUM_DATA_RATE_IEI:
        if ((decoded_result = decode_intergrity_protection_maximum_data_rate(&pdu_session_modification_request->intergrityprotectionmaximumdatarate,
                                                                             PDU_SESSION_MODIFICATION_REQUEST_INTGRITY_PROTECTION_MAXIMUM_DATA_RATE_IEI, buffer + decoded, len - decoded)) < 0)
          return decoded_result;
        else {
          decoded += decoded_result;
      ...
      case PDU_SESSION_MODIFICATION_REQUEST_QOS_FLOW_DESCRIPTIONS_IEI:
        if ((decoded_result = decode_qos_flow_descriptions(&pdu_session_modification_request->qosflowdescriptions, PDU_SESSION_MODIFICATION_REQUEST_QOS_FLOW_DESCRIPTIONS_IEI, buffer + decoded,
                                                           len - decoded)) < 0)
          return decoded_result;
        else {
          decoded += decoded_result;
      ...
      case PDU_SESSION_MODIFICATION_REQUEST_E_P_C_O_IEI:
        if ((decoded_result = decode_extended_protocol_configuration_options(&pdu_session_modification_request->extendedprotocolconfigurationoptions, PDU_SESSION_MODIFICATION_REQUEST_E_P_C_O_IEI,
                                                                             buffer + decoded, len - decoded)) < 0)
          return decoded_result;
        else {
          decoded += decoded_result;
...
int encode_pdu_session_modification_request(pdu_session_modification_request_msg *pdu_session_modification_request, uint8_t *buffer, uint32_t len) {
  ...
  if ((pdu_session_modification_request->presence & PDU_SESSION_MODIFICATION_REQUEST_MAXIMUM_NUMBER_OF_SUPPORTED_PRESENCE) == PDU_SESSION_MODIFICATION_REQUEST_MAXIMUM_NUMBER_OF_SUPPORTED_PRESENCE) {
    if ((encoded_result = encode_maximum_number_of_supported_packet_filters(pdu_session_modification_request->maximumnumberofsupportedpacketfilters,
                                                                            PDU_SESSION_MODIFICATION_REQUEST_MAXIMUM_NUMBER_OF_SUPPORTED_IEI, buffer + encoded, len - encoded)) < 0)
      return encoded_result;
    else
      encoded += encoded_result;
  }

  if ((pdu_session_modification_request->presence & PDU_SESSION_MODIFICATION_REQUEST_ALWAYSON_PDU_SESSION_REQUESTED_PRESENCE)
      == PDU_SESSION_MODIFICATION_REQUEST_ALWAYSON_PDU_SESSION_REQUESTED_PRESENCE) {
    if ((encoded_result = encode_alwayson_pdu_session_requested(pdu_session_modification_request->alwaysonpdusessionrequested, PDU_SESSION_MODIFICATION_REQUEST_ALWAYSON_PDU_SESSION_REQUESTED_IEI,
  ...
  if ((pdu_session_modification_request->presence & PDU_SESSION_MODIFICATION_REQUEST_INTGRITY_PROTECTION_MAXIMUM_DATA_RATE_PRESENCE)
      == PDU_SESSION_MODIFICATION_REQUEST_INTGRITY_PROTECTION_MAXIMUM_DATA_RATE_PRESENCE) {
    if ((encoded_result = encode_intergrity_protection_maximum_data_rate(pdu_session_modification_request->intergrityprotectionmaximumdatarate,
                                                                         PDU_SESSION_MODIFICATION_REQUEST_INTGRITY_PROTECTION_MAXIMUM_DATA_RATE_IEI, buffer + encoded, len - encoded)) < 0)
      return encoded_result;
    else
      encoded += encoded_result;
  }
  ...
  if ((pdu_session_modification_request->presence & PDU_SESSION_MODIFICATION_REQUEST_QOS_FLOW_DESCRIPTIONS_PRESENCE) == PDU_SESSION_MODIFICATION_REQUEST_QOS_FLOW_DESCRIPTIONS_PRESENCE) {
    if ((encoded_result = encode_qos_flow_descriptions(pdu_session_modification_request->qosflowdescriptions, PDU_SESSION_MODIFICATION_REQUEST_QOS_FLOW_DESCRIPTIONS_IEI, buffer + encoded,
                                                       len - encoded)) < 0)
      return encoded_result;
    else
      encoded += encoded_result;
  ...
  if ((pdu_session_modification_request->presence & PDU_SESSION_MODIFICATION_REQUEST_E_P_C_O_PRESENCE) == PDU_SESSION_MODIFICATION_REQUEST_E_P_C_O_PRESENCE) {
    if ((encoded_result = encode_extended_protocol_configuration_options(pdu_session_modification_request->extendedprotocolconfigurationoptions, PDU_SESSION_MODIFICATION_REQUEST_E_P_C_O_IEI,
                                                                         buffer + encoded, len - encoded)) < 0)
      return encoded_result;
    else
      encoded += encoded_result;
