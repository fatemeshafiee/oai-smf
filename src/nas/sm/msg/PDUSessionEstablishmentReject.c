  ...
  while (len - decoded > 0) {
    ...
    switch (ieiDecoded) {
      ...
      case PDU_SESSION_ESTABLISHMENT_REJECT_E_P_C_O_IEI:
        if ((decoded_result = decode_extended_protocol_configuration_options(&pdu_session_establishment_reject->extendedprotocolconfigurationoptions, PDU_SESSION_ESTABLISHMENT_REJECT_E_P_C_O_IEI,
      ...
      case PDU_SESSION_ESTABLISHMENT_REJECT__5GSM_CONGESTION_REATTEMPT_INDICATOR_IEI:
        if ((decoded_result = decode__5gsm_congestion_reattempt_indicator(&pdu_session_establishment_reject->_5gsmcongestionreattemptindicator,
                                                                          PDU_SESSION_ESTABLISHMENT_REJECT__5GSM_CONGESTION_REATTEMPT_INDICATOR_IEI, buffer + decoded, len - decoded)) < 0)
          return decoded_result;
        else {
          decoded += decoded_result;
...
int encode_pdu_session_establishment_reject(pdu_session_establishment_reject_msg *pdu_session_establishment_reject, uint8_t *buffer, uint32_t len) {
  ...
  if ((pdu_session_establishment_reject->presence & PDU_SESSION_ESTABLISHMENT_REJECT_E_P_C_O_PRESENCE) == PDU_SESSION_ESTABLISHMENT_REJECT_E_P_C_O_PRESENCE) {
    if ((encoded_result = encode_extended_protocol_configuration_options(pdu_session_establishment_reject->extendedprotocolconfigurationoptions, PDU_SESSION_ESTABLISHMENT_REJECT_E_P_C_O_IEI,
  ...
  if ((pdu_session_establishment_reject->presence & PDU_SESSION_ESTABLISHMENT_REJECT__5GSM_CONGESTION_REATTEMPT_INDICATOR_PRESENCE)
      == PDU_SESSION_ESTABLISHMENT_REJECT__5GSM_CONGESTION_REATTEMPT_INDICATOR_PRESENCE) {
    if ((encoded_result = encode__5gsm_congestion_reattempt_indicator(pdu_session_establishment_reject->_5gsmcongestionreattemptindicator,
                                                                      PDU_SESSION_ESTABLISHMENT_REJECT__5GSM_CONGESTION_REATTEMPT_INDICATOR_IEI, buffer + encoded, len - encoded)) < 0)
      return encoded_result;
    else
      encoded += encoded_result;
  }
