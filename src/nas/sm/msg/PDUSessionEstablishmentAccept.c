...
while (len - decoded > 0) {
  ...
  case PDU_SESSION_ESTABLISHMENT_ACCEPT_EPCO_IEI:
    if ((decoded_result = decode_extended_protocol_configuration_options (&pdu_session_establishment_accept->extendedprotocolconfigurationoptions, PDU_SESSION_ESTABLISHMENT_ACCEPT_EPCO_IEI, buffer + decoded, len - decoded)) < 0)
      return decoded_result;
    else {
      decoded += decoded_result;
...
*/
  ...
  {
    if((encoded_result = encode_extended_protocol_configuration_options (pdu_session_establishment_accept->extendedprotocolconfigurationoptions, PDU_SESSION_ESTABLISHMENT_ACCEPT_EPCO_IEI, buffer+encoded,len-encoded))<0)
      return encoded_result;
    else
      encoded += encoded_result;
  }
