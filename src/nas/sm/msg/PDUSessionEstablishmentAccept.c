  ...
  while (len - decoded > 0) {
    ...
    switch (ieiDecoded & 0xf0) {
      case PDU_SESSION_ESTABLISHMENT_ACCEPT_ALWAYSON_PDU_SESSION_INDICATION_IEI:
        if ((decoded_result = decode_alwayson_pdu_session_indication(&pdu_session_establishment_accept->alwaysonpdusessionindication,
                                                                     PDU_SESSION_ESTABLISHMENT_ACCEPT_ALWAYSON_PDU_SESSION_INDICATION_IEI, buffer + decoded, len - decoded)) < 0)
          return decoded_result;
        else {
          decoded += decoded_result;
    ...
    switch (ieiDecoded) {
      ...
      case PDU_SESSION_ESTABLISHMENT_ACCEPT_QOS_FLOW_DESCRIPTIONS_IEI:
        if ((decoded_result = decode_qos_flow_descriptions(&pdu_session_establishment_accept->qosflowdescriptions, PDU_SESSION_ESTABLISHMENT_ACCEPT_QOS_FLOW_DESCRIPTIONS_IEI, buffer + decoded,
                                                           len - decoded)) < 0)
          return decoded_result;
        else {
          decoded += decoded_result;
      ...
      case PDU_SESSION_ESTABLISHMENT_ACCEPT_EPCO_IEI:
        if ((decoded_result = decode_extended_protocol_configuration_options(&pdu_session_establishment_accept->extendedprotocolconfigurationoptions, PDU_SESSION_ESTABLISHMENT_ACCEPT_EPCO_IEI,
                                                                             buffer + decoded, len - decoded)) < 0)
          return decoded_result;
        else {
          decoded += decoded_result;
          pdu_session_establishment_accept->presence |= PDU_SESSION_ESTABLISHMENT_ACCEPT_EPCO_PRESENCE;
        }
        break;
...
int encode_pdu_session_establishment_accept(pdu_session_establishment_accept_msg *pdu_session_establishment_accept, uint8_t *buffer, uint32_t len) {
  ...
  if ((pdu_session_establishment_accept->presence & PDU_SESSION_ESTABLISHMENT_ACCEPT_ALWAYSON_PDU_SESSION_INDICATION_PRESENCE)
      == PDU_SESSION_ESTABLISHMENT_ACCEPT_ALWAYSON_PDU_SESSION_INDICATION_PRESENCE) {
    if ((encoded_result = encode_alwayson_pdu_session_indication(pdu_session_establishment_accept->alwaysonpdusessionindication, PDU_SESSION_ESTABLISHMENT_ACCEPT_ALWAYSON_PDU_SESSION_INDICATION_IEI,
  ...
  if ((pdu_session_establishment_accept->presence & PDU_SESSION_ESTABLISHMENT_ACCEPT_QOS_FLOW_DESCRIPTIONS_PRESENCE) == PDU_SESSION_ESTABLISHMENT_ACCEPT_QOS_FLOW_DESCRIPTIONS_PRESENCE) {
    if ((encoded_result = encode_qos_flow_descriptions(pdu_session_establishment_accept->qosflowdescriptions, PDU_SESSION_ESTABLISHMENT_ACCEPT_QOS_FLOW_DESCRIPTIONS_IEI, buffer + encoded,
                                                       len - encoded)) < 0)
      return encoded_result;
    else
      encoded += encoded_result;
  ...
  if ((pdu_session_establishment_accept->presence & PDU_SESSION_ESTABLISHMENT_ACCEPT_EPCO_PRESENCE) == PDU_SESSION_ESTABLISHMENT_ACCEPT_EPCO_PRESENCE) {
    if ((encoded_result = encode_extended_protocol_configuration_options(pdu_session_establishment_accept->extendedprotocolconfigurationoptions, PDU_SESSION_ESTABLISHMENT_ACCEPT_EPCO_IEI,
                                                                         buffer + encoded, len - encoded)) < 0)
      return encoded_result;
    else
      encoded += encoded_result;
