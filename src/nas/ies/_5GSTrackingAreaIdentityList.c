...
int encode__5gs_tracking_area_identity_list(_5GSTrackingAreaIdentityList _5gstrackingareaidentitylist, uint8_t iei, uint8_t *buffer, uint32_t len) {
  ...
  struct PartialTrackingAreaIdentityList *ptr = _5gstrackingareaidentitylist.partialTrackingAreaIdentityList;
  ...
  for (; listIndex < _5gstrackingareaidentitylist.listSize; listIndex++) {
    ...
    switch ((ptr->typeOfList) & 0x03) {
      ...
      case LIST_OF_TAIS_BELONGING_TO_DIFFERENT_PLMNS:
        ...
        for (; elementIndex < ptr->numberOfElements; elementIndex++) {
          if (!ptrTai3 | !ptrMccMnc)
            break;
          octet = (uint8_t)((ptrMccMnc->mcc) & 0x00ff);
          ENCODE_U8(buffer + encoded, octet, encoded);
          octet = (uint8_t)(((ptrMccMnc->mcc) & 0x0f00) >> 8) | (uint8_t)(((ptrMccMnc->mnc) & 0x0f00) >> 4);
          ENCODE_U8(buffer + encoded, octet, encoded);
