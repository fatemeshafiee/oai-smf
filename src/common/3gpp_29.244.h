...
struct pfcp_tlv_exception : public pfcp_ie_exception {
 public:
  pfcp_tlv_exception(uint16_t ie_type) throw ()
      :
      pfcp_ie_exception(ie_type) {
...
typedef struct pdr_id_s {
  ...
  pdr_id_s(const uint8_t &p)
      :
      rule_id(p) {
  }
  pdr_id_s(const struct pdr_id_s &p)
      :
      rule_id(p.rule_id) {
  }
...
struct far_id_s {
  ...
  far_id_s(const uint8_t &f)
      :
      far_id(f) {
  }
  far_id_s(const struct far_id_s &f)
      :
      far_id(f.far_id) {
  }
...
typedef struct qfi_s {
  ...
  qfi_s(const uint8_t &q)
      :
      qfi(q),
      spare(0) {
  }
  qfi_s(const struct qfi_s &q)
      :
      qfi(q.qfi),
      spare(q.spare) {
  }
