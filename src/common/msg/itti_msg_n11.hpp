...
class itti_n11_create_sm_context_request : public itti_n11_msg {
 public:
  itti_n11_create_sm_context_request(const task_id_t orig, const task_id_t dest, Pistache::Http::ResponseWriter& response):
    itti_n11_msg(N11_SESSION_CREATE_SM_CONTEXT_REQUEST, orig, dest), http_response(response), req(), scid() {}
  itti_n11_create_sm_context_request(const itti_n11_create_sm_context_request& i) : itti_n11_msg(i), req(i.req), http_response(i.http_response), scid()  {}
...
class itti_n11_update_sm_context_response : public itti_n11_msg {
 public:
  itti_n11_update_sm_context_response(const task_id_t orig, const task_id_t dest, Pistache::Http::ResponseWriter& response):
    itti_n11_msg(N11_SESSION_UPDATE_SM_CONTEXT_RESPONSE, orig, dest), http_response(response.clone()), res(), session_procedure_type() {}
  itti_n11_update_sm_context_response(const itti_n11_update_sm_context_response& i) : itti_n11_msg(i), res(i.res), http_response(i.http_response.clone()), session_procedure_type(i.session_procedure_type)  {}
  itti_n11_update_sm_context_response(const itti_n11_update_sm_context_response& i, const task_id_t orig, const task_id_t dest) :
    itti_n11_msg(i, orig, dest), res(i.res), http_response(i.http_response.clone()), session_procedure_type(i.session_procedure_type) {}
