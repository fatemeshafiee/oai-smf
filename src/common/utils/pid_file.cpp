...
bool util::is_pid_file_lock_success(const char * pid_file_name)
{
  ...
  if ( 0 > g_fd_pid_file) {
    Logger::pgwc_app().error( "open filename %s failed %d:%s\n", pid_file_name, errno, strerror(errno));
    return false;
  }

  if ( 0 > util::lockfile(g_fd_pid_file, F_TLOCK)) {
    Logger::pgwc_app().error( "lockfile filename %s failed %d:%s\n", pid_file_name, errno, strerror(errno));
  ...
  if (ftruncate(g_fd_pid_file, 0)) {
    Logger::pgwc_app().error( "truncate %s failed %d:%s\n", pid_file_name, errno, strerror(errno));
    close(g_fd_pid_file);
    return false;
  }
  // write PID in file
  ...
  if ((ssize_t)-1 == write(g_fd_pid_file, pid_dec, strlen(pid_dec))) {
    Logger::pgwc_app().error( "write PID to filename %s failed %d:%s\n", pid_file_name, errno, strerror(errno));
    return false;
  }
  return true;
}
